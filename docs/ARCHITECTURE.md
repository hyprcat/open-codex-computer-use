...
- MCP `tools/list` 的 description / input schema 按官方 `computer-use` 的 9 个 tools 文案和参数面收敛，尽量减少 host 侧提示词和 tool surface 偏差。
- macOS 在这 9 个之外额外提供一组窗口准备 tool：`prepare_agent_display`、`prepare_app`、`restore_prepared_window`、`close_prepared_window`。`prepare_app` 把目标窗口移到 agent display 上，让 agent 不必和用户抢屏幕；结束时 `restore_prepared_window` 把窗口放回用户原来的位置，`prepare_app` 自己打开的窗口则按它自己的关闭按钮关掉，不牵动应用的其他窗口。全屏 Space 里的窗口一律不碰：重新打开会把用户切到那个 Space，而全屏窗口也搬不到 agent display 上。每次退出都会把前台进程还原回调用前的那个。
...
- 动作型 tools 对普通 app 采用“非侵入优先，物理指针路径显式 opt-in”策略：
  ...
  - `get_app_state` 解析目标窗口时先看 on-screen 窗口，再退到所有窗口，所以位于其他 Space 的窗口不会触发 activate / raise 恢复；拍到未被遮挡的窗口后会关闭它的 WindowServer occlusion 通知（`WindowOcclusionKeepAlive`），之后被遮挡或切换 Space 时 Chromium / Electron / WebKit 仍保留页面与 `AXWebArea`。keep-alive 保存 SPI 返回的原始开关状态，并在 MCP server 正常关闭或进程退出时恢复；恢复失败的记录保留到后续重试。Chromium 系 app 的 web AX tree 是懒加载的，窗口可见时最多重走 3 秒，被遮挡时在 tree 末尾追加说明。`sky_click` 不再要求窗口 on-screen。详见 `docs/references/macos-window-visibility-and-spaces.md`。
  - 窗口截图主路径是 WindowServer 的 `SLSHWCaptureWindowList`（任何 Space、被遮挡均可，15–45 ms），ScreenCaptureKit 为 fallback；AX root window 通过 `_AXUIElementGetWindow` 直接绑定 `CGWindowID`。输入 recipe 的固定间隔都是环境变量可调的校准旋钮，默认值来自基准测试（见 `docs/references/macos-window-visibility-and-spaces.md`）。
  - `get_app_state.window_placement` 是开源版的可选扩展：`keep`（默认）、`agent_display`、`restore`。`agent_display` 通过 `AgentDisplay` + `packages/OpenComputerUseVirtualDisplayShim` 创建 agent 专用 virtual display，用 AX 位置把目标窗口的 frame 移过去（不激活、不抬升），让已被遮挡或在其他 Space 的窗口对其 app 真正可见；`restore`、MCP server 正常关闭或进程退出时移回原位并销毁显示器。恢复失败时保留原位置记录和 virtual display，避免把未恢复窗口误记为成功并允许后续重试；已关闭的窗口则直接清除记录。Windows / Linux 对非 `keep` 值返回 unsupported。
  - `type_text.key_method` / `press_key.key_method` 是开源版的可选扩展，支持 `auto`（默认）和 macOS-only 的 `sky_key`。`sky_key` 复用 `sky_click` 的 target-only synthetic-active record，再投递 yabai 的 make-key-window record 让目标 app 在进程内把当前 snapshot 窗口设为 key window，之后走现有 `CGEvent.postToPid` 键盘路径；带 `cmd` 的单字符组合键改为在目标 AX 菜单栏找到对应菜单项并 `AXPress`。它不要求窗口 on-screen（可用于其他 Space），但对隐藏 app fail closed；action-result snapshot 与 `sky_click` 一样使用 read-only recovery policy。
...
- Windows `click_method=accessibility` 映射到 UI Automation pattern，`app_post` 映射到 HWND `PostMessage`；macOS-only 的 `sky_click` 和没有实现的 `global` 都会在 snapshot lookup 前明确返回 unsupported。`type_text` / `press_key` 的 `key_method=sky_key` 同样在 snapshot lookup 前返回 unsupported。`auto` 仍保持 UIA 优先、window message fallback 的现有行为。
...
- Linux `click_method=accessibility` 映射到 AT-SPI action，`global` 映射到 AT-SPI mouse synthesis 并要求全局指针环境变量；AT-SPI 没有等价的进程定向 mouse dispatch，因此 `app_post` 和 macOS-only 的 `sky_click` 会在 snapshot lookup 前明确返回 unsupported，`key_method=sky_key` 也一样。`auto` 仍保持 AT-SPI action 优先、mouse synthesis fallback 的现有行为。
