...
- 动作型 tools 对普通 app 采用“非侵入优先，物理指针路径显式 opt-in”策略：
  ...
  - `get_app_state.window_placement` 是开源版的可选扩展：`keep`（默认）、`agent_display`、`restore`。`agent_display` 通过 `AgentDisplay` + `packages/OpenComputerUseVirtualDisplayShim` 创建 agent 专用 virtual display，用 AX 位置把目标窗口的 frame 移过去（不激活、不抬升），让已被遮挡或在其他 Space 的窗口对其 app 真正可见；`restore` 或进程退出时移回原位并销毁显示器。Windows / Linux 对非 `keep` 值返回 unsupported。
