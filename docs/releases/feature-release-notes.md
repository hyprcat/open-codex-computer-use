...
| 日期 | 功能域 | 用户价值 | 变更摘要 |
| --- | --- | --- | --- |
| 2026-09-15 | macOS 窗口准备 | agent 可以在不占用用户屏幕的前提下操作应用：目标窗口被移到 agent display 上工作，结束后放回原处，前台焦点始终还给用户。 | macOS 新增 `prepare_agent_display` / `prepare_app` / `restore_prepared_window` / `close_prepared_window`：支持按菜单栏 New Window 开新窗口、对只剩关闭窗口的应用发 reopen、跳过全屏 Space 中的窗口，并在每次退出时还原前台进程。 |
